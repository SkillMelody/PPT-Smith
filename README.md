...
模板也可以仅作为风格参考：明确允许重新设计时使用 `create`，可选“参考模板图 → 生图设计稿 → 原生对象重建”；无生图能力继续采用原生设计与真实预览。严格保留母版和原生模板资产仍使用 `template`。详见[模板风格设计与使用边界](references/template-visual-design.md)。
